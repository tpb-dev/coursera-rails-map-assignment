(function() {
  ...
  function CurrentSubjects($rootScope, $q, $resource, currentOrigin, APP_CONFIG) {
    ...
    // service.handleMarkers = handleMarkers;
...
//       if(window && window.mappy) {
        


//         if (window.mappy.currentMarker) {
//           console.log("We have current Marker here")
// //          window.mappy.clearMarkers()
//           window.mappy.setActiveMarker(window.mappy.currentMarker)
// //          window.mappy.displayMarker(window.mappy.currentMarker)
...
//     }
    ...
    function refresh() {      
      ...
      function refresh2(panama) {
          ...
          params["order"]="ASC";
          console.log("refresh2",params);

          var p1=refreshImages(params);
          params["subject"]="thing";
          params["q"] = panama.q;
          var p2=searchThings(params);
          $q.all([p1,p2]).then(
              function(){
                  service.setCurrentImageForCurrentThing();
                  // handleMarkers()   
      ...
      function searchThings(params) {
          ...
          result.$promise.then(
              function(things){
                  ...
                  console.log("searchThings", service);
                  //handleMarkers()
              });
          return result.$promise;
      }
