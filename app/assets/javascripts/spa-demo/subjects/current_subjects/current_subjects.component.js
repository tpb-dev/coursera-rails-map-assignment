(function() {
  ...
  function CurrentSubjectsMapController($scope, $q, $element, 
                                        ...
                                        APP_CONFIG) {
    var vm=this;
    window.currentSubjects = this;
    window.displaySubject = displaySubject;
    window.mappTiToMarkerOptions = new Object();
    window.getSubjectFromMap = getSubjectFromMap;
    window.getSubjectFromMapViaId = getSubjectFromMapViaId;
    ...
    vm.$postLink = function() {
      ...
      $scope.$watch(
        ...
        function(images) {
          vm.images = images;
          displaySubjects();
          window.mappy.clearOriginMarker()
        }); 
      $scope.$watch(
      ...
      $scope.$watch(
        ...
        function(thing) { 
          if (thing) { 
            vm.setActiveMarker(thing.thing_id, thing.image_id);
            window.mappy.clearMarkers()
            window.mappy.clearOriginMarker()
            window.displaySubject(window.getSubjectFromMap(thing)) 
            vm.setActiveMarker(thing.thing_id, thing.image_id);
          } else {
            vm.setActiveMarker(null,null);           
          }
    ...
    function getSubjectFromMap(thang) {
      return window.mappTiToMarkerOptions[thang.thing_id]
    }

    function getSubjectFromMapViaId(id) {
      return window.mappTiToMarkerOptions[id]
    }
    ...
    function initializeMap(element, position) {
      ...
      displaySubjects();  
      if(!window.mappy.currentMarker && window.mappy.markers.length > 0) {
          window.mappy.currentMarker = window.mappy.markers[0]
          
          window.mappy.clearOriginMarker()
          //window.mappy.clearMarkers()
          // window.mappy.setActiveMarker(window.mappy.currentMarker)
          //displaySubject(window.mappy.currentMarker);
          console.log("before End loading")
          window.mappy.clearMarkers()
          console.log("before End loading")
          window.displaySubject(window.getSubjectFromMapViaId(window.mappy.currentMarker.thing_id)) 
          vm.setActiveMarker(window.mappy.currentMarker.thing_id, window.mappy.currentMarker.image_id);
          //displayMarker(window.mappy.currentMarker)
        }
    ...
    function displaySubjects(){
      ...
      angular.forEach(vm.images, function(ti){
        ...
        console.log("Current marker = " + window.mappy.currentMarker)
        displaySubject(ti);
      });
    }
    ...
    function displaySubject(ti) {
      ...
      if(ti.thing_id) {
        window.mappTiToMarkerOptions[ti.thing_id] =  ti;
      }
