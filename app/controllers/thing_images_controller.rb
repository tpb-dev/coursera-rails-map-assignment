class ThingImagesController < ApplicationController
  ...
  def subjects
    ...
    if stale?  :etag=>eTag
      ...
      a=ThingImage.query_search(@thing_images, search_params)
      puts(@thing_images.length)
      @thing_images = a
      #@thing_images=a if query else @thing_images
  ...
  def search
    ...
    line = "description ilike ? or name ilike ?", "%" + search_params[:q] + "%", "%" + search_params[:q] + "%"
    @things = Thing.where("description ilike ? or name ilike ?", "%" + search_params[:q] + "%", "%" + search_params[:q] + "%")
    puts(@things.size)
  end
