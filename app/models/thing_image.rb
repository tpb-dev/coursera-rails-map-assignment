...
=end
  ...
  def self.query_search(thing_images, search_params)
    a = thing_images.where("description ilike ? or name ilike ?", "%" + search_params[:q] + "%", "%" + search_params[:q] + "%") if search_params[:q]
    if !search_params[:q]
      a = thing_images
    end
    return a
